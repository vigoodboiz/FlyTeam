...
@section('content')
    ...
    <main class="main__content_wrapper">
        ...
        <!-- cart section start -->
        <div class="container">
            <h2 class="cart__title mb-30">Đơn hàng đã hủy</h2>
            <br>
            <ul class="nav nav-tabs">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="{{ route('canceled') }}">Đã hủy đơn hàng
                        @if ($canceledCount > 0)
            ...
            <br>
            @if ($orders->isEmpty())
                <h4 style="text-align: center; color: red;">Bạn chưa có đơn hàng nào bị hủy cả!</h4>
            @else
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($orders as $item)
                            @if ($item->payment_status === 'Đã hủy đơn hàng' && $item->delivery_status === 'Không thể xử lí giao hàng')
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td>
